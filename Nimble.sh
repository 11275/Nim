...
function root_model_en(){
    # 检查是否以root用户运行脚本
    if [ "$(id -u)" != "0" ]; then
        echo "This script needs to be run with root user privileges."
        echo "Please try to switch to root user with 'sudo -i' command, then run this script again."
        exit 1
    fi
}
...
function create_wallet(){
    ...
    echo -e "\e[0;32m  【*******创建钱包至少要一对，主钱包和子钱包**********】\e[0m"
...
function check_balance(){
    ...
    echo -e "\e[0;32m  【*******第二种是官方提供的查询方式，即本方式**********】\e[0m"
...
function main_menu_ch(){
    ...
    echo "10. 新钱包地址挖矿----适用于未注册过的钱包地址  ) "
    echo "11. 老钱包地址挖矿----适用于已注册过的钱包地址  ) "
    echo "12. 查看当前nim进程  ) "
    echo "13. 关闭nim进程  ) "
    echo "===================================快捷启动（已经安装好了环境）多卡================================================================="
    echo "14. 新钱包地址多卡挖矿----适用于未注册过的钱包地址-----批量模式  ) "
    echo "15. 老钱包地址多卡挖矿----适用于已注册过的钱包地址-----批量模式  ) "
    echo "16. 新钱包地址多卡挖矿----适用于未注册过的钱包地址-----指定某张卡  ) "
    echo "17. 老钱包地址多卡挖矿----适用于已注册过的钱包地址-----指定某张卡  ) "
    echo "18. 查看当前多卡nim进程  ) "
    echo "19. 关闭多卡nim进程 --------批量模式 ) "
    echo "20. 关闭多卡nim进程 --------指定模式 ) "
    echo "21. 关闭多卡nim进程 --------指定模式------加重新启动----新钱包申请 ) "
    echo "22. 关闭多卡nim进程 --------指定模式------加重新启动----老钱包运行 ) "
    echo "=================================================辅助测试工具======================================================================="
    echo "23. 测试显卡驱动 ) "
    echo "24. 测试连接huggingface.co ) "
    echo "25. 显卡驱动安装 ) "
    echo "==================================================建议和参考========================================================================"
    echo "26. 相关参考和建议 "
    echo "27. 参考算力表 "
    ...
    8) check_balance ;;
    9) view_upload ;;
    10) allConfirm=y && start_with_new_one_gpu ;;
    11) allConfirm=y && start_with_old_one_gpu ;;
    12) show_status_one_gpu ;;
    13) close_node_one_gpu ;;
    14) start_with_new_gpus ;;
    15) start_with_old_gpus ;;
    16) start_with_new_gpus_appoint ;;
    17) start_with_old_gpus_appoint ;;
    18) show_status_multiple ;;
    19) close_node_multiple ;;
    20) close_node_appoint ;;
    21) close_node_appoint_new_restart ;;
    22) close_node_appoint_old_restart ;;
    23) test_gpu ;;
    24) test_connection ;;
    25) install_cuda ;;
    26) personal ;;
    27) computing_power_meter ;;
    28) uninstall_node ;;
    *) echo "无效选项。" ;;
    esac
}
...
function guide_ch() {
    clear
    echo -e "\e[0;32m  【***************************tom哥脚本（参考大赌哥以及相关社区大佬创作）****************************************】\e[0m"
    echo -e "\e[0;32m温馨提醒该程序适用于nim ubuntu教程,同样也适用windows下的wsl的ubuntu系统。\e[0m"
    echo -e "\e[0;32m该项目需要cuda驱动的显卡，推荐4090最佳,其他根据自己已有资源自己选择，纯cpu肯定是不行的哦。\e[0m"
    echo -e "\e[0;32m运行程序推荐测试显卡驱动是否安装正常。\e[0m"
    echo -e "\e[0;32m运行程序推荐测试连接huggingface.co是否正常，模型下载，模型训练配置相关获取都是连接的它，网络不通，无法开始挖矿。\e[0m"
    echo -e "\e[0;32m此项目必须翻墙使用，运行相关网络必须稳定，训练结果上传不成功是拿不到任何奖励的。\e[0m"
    echo "******************************************************************************************************************************"
    echo "******************************************************************************************************************************"
    echo "******************************************************************************************************************************"
...
function guide_en() {
    clear
    echo -e "\e[0;32mKind reminder that this program is suitable for the nim ubuntu tutorial and also for the wsl ubuntu system under Windows.\e[0m"
    echo -e "\e[0;32mThis project requires a CUDA driven graphics card, and the best recommendation is 4090.\e[0m"
    echo -e "\e[0;32mIt is recommended to test whether the graphics card driver is installed properly when running the program.\e[0m"
    echo -e "\e[0;32mRunning the program recommends testing whether the connection to huggingface. co is working properly. The model download and training configuration are all connected to it, and the network is not working, making it impossible to start mining.\e[0m"
    echo -e "\e[0;32mThe network related to the operation of this project must be stable, and without uploading the training results, no rewards will be obtained.\e[0m"
...
# # 确定使用的版本
# echo "请选择输出的脚本语言：中文/英文（English），中文输入ch,英文输入en"
# echo "Please select the script language for output: Chinese/English, Chinese input ch, English input en"
# read -p "请输入 / Please enter: " language
# export language
# if [ $language =  ch ]; then
#     echo "你选择了中文版本"
#     root_model_ch
#     guide_ch  
# else 
#     echo "You have chosen the English version" 
#     root_model_en
#     guide_en   
# fi
